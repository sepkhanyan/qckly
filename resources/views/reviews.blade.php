                                ...
                                <select name="restaurant_name" id="input-name" class="form-control" tabindex="-1"
                                    ...
                                    @if($selectedRestaurant)
                                        @foreach($restaurants as $restaurant)
                                                <option value="{{url('/reviews/' . $restaurant->id)}}"{{($restaurant->id == $selectedRestaurant->id) ? 'selected' : ''}}>{{$restaurant->name_en}}</option>
                                        @endforeach
                                    @else
                                        <option value>Select Restaurant</option>
                                        @foreach($restaurants as $restaurant)
                                            <option value="{{url('/reviews/' . $restaurant->id)}}">{{$restaurant->name_en}}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
